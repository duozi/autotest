 ...
 * 
 * <p>
 * 判断如果参数中有 {@link Pagenation} 对象，那么执行分页查询。(1.查询总数并放入page对象中。 2.构造带有limit子句的sql替换原始的sql)
 * </p>
 * <p>
...
public class PageInterceptor implements Interceptor {
    private static final ThreadLocal<Pagenation> PAGE_CONTEXT = new ThreadLocal<Pagenation>();
        ...
        if (ivk.getTarget() instanceof RoutingStatementHandler) {
            ...
            if (param instanceof HashMap) { 
                ...
                if (map.containsKey(PAGE_KEY)){
                    Pagenation p = (Pagenation) map.get(PAGE_KEY);
                    if (p != null) {
                        p.setTotalRecord(queryTotal(ivk, ms, bs, param, sql));
                        set(p);
     ...
     * @return
     */
    public static Pagenation getPage() {
        Pagenation p = PAGE_CONTEXT.get();
        PAGE_CONTEXT.remove();
        return p;
    }
     ...
     * @param p
     */
    private static void set(Pagenation p) {
        PAGE_CONTEXT.set(p);
    }
     ...
     * @return
     */
    private String pageSql(String sql, Pagenation page) {
